...
public class StatAction {
	...
	@RequestMapping("/stat/siteQueryDetail.do")
	public @ResponseBody String siteQueryDetail(@RequestParam("key") String key,
			@RequestParam("startTime") String startTime, @RequestParam("endTime") String endTime,
			@RequestParam(value = "siteTypeId", required = false) Integer siteTypeId,
			@RequestParam(value = "siteId", required = false) Integer siteId,
			@RequestParam(value = "page", required = false) Integer page,
		...
		int startNum = page == null ? 0 : (page - 1) * rows;
		// XXX 需要完成
		int total = 0;//solrService.countWordsQueryDetail(start, end, langId, countryId, siteTypeId,siteId, wordsId);
		List<Map<String, Object>> details = getSiteTestDatas(siteId);//solrService.statWordsQueryDetail(start, end, langId, countryId, siteTypeId,			siteId, wordsId,startNum, rows);
		if (null == details){
			details = Collections.emptyList();
		}
		total = details.size();
	...
	private List<Map<String, Object>> getSiteTestDatas(Integer siteId) {
		Map<String,List<Map<String,Object>>> datas = new HashMap<String, List<Map<String,Object>>>();
		List<Map<String,Object>> sina = new ArrayList<Map<String,Object>>();
		datas.put("33", sina);
		Map<String,Object> data = new HashMap<String, Object>();
		sina.add(data);
		data.put("title", "欧佩克原油产出再创历史纪录");
		data.put("author", "邵青");
		data.put("content", "");
		data.put("source", "大洋网-广州日报");
		data.put("publish_time", "2016-9-6");
		data.put("crawl_time", "2016-09-06 21:25:22");
		data.put("url", "http://www.cpcia.org.cn/html/13/20169/157321.html");
		
		data = new HashMap<String, Object>();
		sina.add(data);
		data.put("title", "扬子石化乙烯产量已超过55万吨");
		data.put("author", "邵青");
		data.put("content", "");
		data.put("source", "中国石化新闻网");
		data.put("publish_time", "2016-9-6");
		data.put("crawl_time", "2016-09-06 21:25:25");
		data.put("url", "http://www.cpcia.org.cn/html/13/20169/157320.html");

		data = new HashMap<String, Object>();
		sina.add(data);
		data.put("title", "《巴黎协定》将推动氢氟碳化物减排");
		data.put("author", "邵青");
		data.put("content", "");
		data.put("source", "财新网");
		data.put("publish_time", "2016-9-6");
		data.put("crawl_time", "2016-09-06 21:25:30");
		data.put("url", "http://www.cpcia.org.cn/html/13/20169/157319.html");
		
		data = new HashMap<String, Object>();
		sina.add(data);
		data.put("title", "俄罗斯准备冻产 沙特却称没有必要");
		data.put("author", "邵青");
		data.put("content", "");
		data.put("source", "北京商报");
		data.put("publish_time", "2016-9-6");
		data.put("crawl_time", "2016-09-06 21:25:33");
		data.put("url", "http://www.cpcia.org.cn/html/13/20169/157318.html");
		
		data = new HashMap<String, Object>();
		sina.add(data);
		data.put("title", "8月下旬全国炼焦煤价格涨幅扩大");
		data.put("author", "邵青");
		data.put("content", "");
		data.put("source", "中国煤炭资源网");
		data.put("publish_time", "2016-9-6");
		data.put("crawl_time", "2016-09-06 21:25:44");
		data.put("url", "http://www.cpcia.org.cn/html/13/20169/157317.html");
		
		data = new HashMap<String, Object>();
		sina.add(data);
		data.put("title", "盐城市政府动真碰硬 全力整治化工企业消防隐患");
		data.put("author", "邵青");
		data.put("content", "");
		data.put("source", "盐城市消防");
		data.put("publish_time", "2016-9-6");
		data.put("crawl_time", "2016-09-06 21:25:55");
		data.put("url", "http://www.cpcia.org.cn/html/13/20169/157316.html");
		
		List<Map<String,Object>> w163 = new ArrayList<Map<String,Object>>();
		datas.put("3", w163);
		return datas.get(String.valueOf(siteId));
	}
