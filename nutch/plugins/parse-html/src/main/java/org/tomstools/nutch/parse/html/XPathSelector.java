...
public class XPathSelector {
	...
	/** 值类型：时间 */
	public static final String VALUE_TYPE_DATATIME = "datetime";
	/** 值类型：相对时间 */
	public static final String VALUE_TYPE_RELATIVE_TIME = "relative_time";
	...
	private ValueTransferable transfer;
	public XPathSelector(String xPathExp) {
		this(xPathExp,VALUE_TYPE_STRING,"");
	}

	public XPathSelector(String xPathExp,String valueType,String valueFormat) {
		xPather = new XPather(xPathExp);
		if (VALUE_TYPE_DATATIME.equalsIgnoreCase(valueType)){
			//  时间格式
			transfer = new DateTimeTransfer(valueFormat);
		}else if (VALUE_TYPE_RELATIVE_TIME.equalsIgnoreCase(valueType)){
			// 相对时间
			// XXX 待完善
...
//	}
	...
	public String evaluate2string(TagNode node, String separator) {
		...
		if (null != result && result.length != 0) {
			...
			return null != transfer ? transfer.transfer(msg.toString()) : msg.toString();
		} else {
			return null;
		}
	}
